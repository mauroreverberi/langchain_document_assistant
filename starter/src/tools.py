...
def create_calculator_tool(logger: ToolLogger):
    ...
    def calculator(expression: str) -> str:
        """Calculate a basic math expression and return the result as a string.

        Args:
            expression: a math expression with numbers and the operators
                + - * / % and parentheses, e.g. "2 + 3" or "(5000 + 12500) * 1.1".

        Returns:
            The result as a string, or an error message if something is wrong.
        """
        # only allow digits, spaces, the basic operators, parentheses and dots
        allowed = set("0123456789+-*/%(). ")
        clean = expression.replace(",", "")  # so "1,000" works too

        try:
            if not clean.strip():
                return "Error: the expression is empty."
            if not set(clean) <= allowed:
                msg = "Error: only numbers and + - * / % ( ) are allowed."
                logger.log_tool_use("calculator", {"expression": expression}, {"error": msg})
                return msg

            result = eval(clean)

            # show "5" instead of 5.0
            ...
            msg = "Error: you can't divide by zero."
            logger.log_tool_use("calculator", {"expression": expression}, {"error": msg})
            return msg
        except Exception as e:
            msg = f"Error: could not calculate that ({e})."
            logger.log_tool_use("calculator", {"expression": expression}, {"error": msg})
            return msg
